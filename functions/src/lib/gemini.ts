...
// Spiegel von src/types.ts → FoodProfile bzw. src/services/ai.ts →
// RecipeContext. Bewusst dupliziert: die Functions teilen keinen Code mit dem
// Client-Bundle.
...
// ── Paar-Coach ───────────────────────────────────────────────────
// Ersetzt den früheren Ein-Satz-Finanz-Insight. Das Schema erzwingt Struktur,
// weil genau die den Unterschied macht: `suggestion.action` verwandelt einen
// Text in einen Tap, und alle Ziele existieren in der App bereits.
export type CoachLens = 'week' | 'fairness' | 'money'

export const COACH_LENSES: CoachLens[] = ['week', 'fairness', 'money']
...
export interface CoachReport {
  headline: string
  sections: { id: string; title: string; text: string; tone: string }[]
...
export async function generateCoachInsight(lens: CoachLens, snapshot: unknown): Promise<CoachReport> {
...
${CHECKIN_RULES}`
  ...
  if (!parsed?.headline) {
    throw new HttpsError('unavailable', 'Die KI hat keine Auswertung geliefert.')
  }
  return parsed
}
...
async function callGemini(prompt: string, schema: object, model = MODEL): Promise<string> {
  ...
  if (!res.ok) {
    ...
    // 429 ist Geminis eigenes Ratenlimit, nicht unsere Paar-Quote — und es gibt
    // zwei davon: ein kurzes Minutenlimit und ein Tageskontingent fürs ganze
    // Google-Projekt. Der Client unterscheidet sie am Wortlaut.
    if (res.status === 429) {
      const daily = /PerDay|RequestsPerDay/i.test(body)
      throw new HttpsError(
        'unavailable',
        daily
          ? 'Das Tageskontingent der KI ist aufgebraucht (429). Morgen wieder verfügbar.'
          : 'Die KI ist gerade ausgelastet (429). Bitte gleich noch einmal versuchen.'
      )
    }
  ...
  if (candidate?.finishReason === 'MAX_TOKENS') {
    console.error('Gemini: Antwort abgeschnitten (MAX_TOKENS)')
    throw new HttpsError('unavailable', 'Die Antwort wurde abgeschnitten. Versuch es mit weniger Tagen auf einmal.')
  }
  if (!text) {
    console.error(`Gemini: leere Antwort (finishReason=${candidate?.finishReason ?? 'unbekannt'})`)
...
// src/services/aiDirect.ts bleiben.
...
export type WeekPlanInput = RecipeContext & { count: number }
