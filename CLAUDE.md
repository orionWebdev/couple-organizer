...
`src/router/index.ts` has a single `beforeEach` guard that awaits `authReady` and redirects based on two route meta flags: `requiresAuth` (must be signed in) and `requiresCouple` (must have a `coupleId`, otherwise sent to `/couple-setup`). The five main tabs — **`/dashboard` (Start, default landing) · `/haushalt` · `/planung` (labelled "Wir") · `/finanzen` · `/einkaufen` (labelled "Küche")** — are children of `TabsView.vue`, which renders the bottom tab bar and hosts cross-tab UI (`Toast`, the invite-partner `BottomSheet`, `PaywallSheet`). The "Wir" tab deliberately keeps `id: 'planung'` and `/planung` (no route churn, persisted deeplinks stay valid); only label + nav icon (the brand's Overlap mark in `NavIcon.vue`) changed.

There is **no `/belegung` route** — the month calendar lives inside the Wir tab's "Planung" segment (`BelegungKalender.vue`, its only call site), reached from the Dashboard via `/planung?tab=planung`. `SLOT_ALIASES` in `TabsView.vue` is consequently an empty map; it exists only as the hook for a future slot-less route.
...
**Only the active slot is labelled** (decided 2026-07-22, after the Wir-Umbau made five labels feel busy — the bubble already says where you are). `.mnav__label` is hidden with `opacity: 0` alone, deliberately not `display:none`/`visibility:hidden`: staying in the layout keeps the icon row from jumping on every switch (only the active icon lifts by −24px into the bubble), and staying in the accessibility tree keeps all five targets named for screen readers.
